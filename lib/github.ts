...
export async function fetchContentFromGitHub(
  ...
  try {
    ...
    if (isDev && isServer) {
      try {
        // Use eval to dynamically require server-only modules
        // This prevents bundlers from statically resolving these imports
        const fsModule = await eval('import("fs/promises")')
        const pathModule = await eval('import("path")')
        const fs = fsModule
...
function parseMarkdownContent(
  ...
  if (contentType === "recipes") {
    ...
    // Parse instructions - try JSON array first, then parse numbered list
      ...
      try {
        ...
        if (typeof frontmatter.instructions === 'string') {
          instructions = frontmatter.instructions
            .split("\n")
            .map((inst: string) => inst.replace(/^\s*\d+\.\s*/, '').trim())
